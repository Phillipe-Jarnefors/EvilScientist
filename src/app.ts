...
let evilObject: {
	evilScientists: Evils[],
	printListOfScientists: ()=> void,
	addValueFromForm: ()=> void,
} = {	
		evilScientists: [
		{
			name: "Klaus Benedict",
	...
	printListOfScientists(): void {
		...
		let evilScientistsLength: number = this.evilScientists.length;
		for (let i = 0; i < evilScientistsLength; i++) {
			...
			divElem.addEventListener("click", (event) => {
				...
				const choosenEvil = this.evilScientists.find(evil => evil.name === pickedEvil) as Evils;
